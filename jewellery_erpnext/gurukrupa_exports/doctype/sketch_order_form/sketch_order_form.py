...
def apply_order_criteria_dates(parent, sketch_order, order_criteria):
    ...
    for criteria in order_criteria.order:
        ...
        ibm_delivery = calculate_ibm_delivery(
            sketch_delivery, criteria.skecth_approval_timefrom_ibm_team, shift_start, shift_end
        )
...
def create_purchase_order(doc):
    ...
    frappe.msgprint(
        _("The following {0} is created: {1}").format(
            frappe.bold(_("Purchase Order")), "<br>" + get_link_to_form("Purchase Order", po.name)
        )
    )
